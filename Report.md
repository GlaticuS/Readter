...
Также задается матрица начального распределения вероятностей (Initial State Distribution Matrix) - точнее, вектор начального распределения вероятностей - показывает, с какойвероятностью объект находится в состоянии A1, A2, A3...
...
[![markov_simple_empty.png](https://s30.postimg.org/qedsdsiup/markov_simple_empty.png)](https://postimg.org/image/nkan0cgod/)
...
Для этого мы можем соорудить дерево вероятностей переходов из A в A' и отдельно посчитать, какова на следующем шаге будет вероятность того, что объект будет находиться в состоянии A или A' - это и показывает вектор S1[P(A), P(A')]. Найдём его компоненты.
...
Но данную величину можно было посчитать другим способом, а именно, перемножив вектор начального распределения на матрицу переходных вероятностей (S0\*P). Можно проверить и получить такой же ответ. 
...
#### Список источников и полезные ссылки
- http://setosa.io/ev/markov-chains/ - визуальное описание Марковских цепей, очень красиво и наглядно.
- https://www.youtube.com/user/patrickJMT - канал на YouTube с видео математической тематики, откуда я черпала информацию по Марковским цепям.
- https://github.com/linvi/tweetinvi - репозиторий библиотеки Tweetinvi
- https://msdn.microsoft.com/ru-ru - справка Microsoft по её продкутам.
